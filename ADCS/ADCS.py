...
while 1:
	...
	rawYaw[i] = float( orientationString[roll] ) + 180		# Since we are using only the yaw, we convert it to a float and store it : range [0,360]
	...
	deltaSum += deltaDrift
	currentYaw[i] = ( rawYaw[i] + deltaDrift ) % 360
